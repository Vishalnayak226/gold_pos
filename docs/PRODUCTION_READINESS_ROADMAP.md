...
- [~] Runbooks: day open/close, payment mismatch, invoice duplicate, rate outage, connectivity loss, device loss, termination, restore and incident. *(**Eight of the nine written 2026-08-17, Phase 34 — `docs/RUNBOOKS.md`.** Day open/close, payment mismatch, duplicate/missing invoice number, rate outage, connectivity loss, lost or stolen device, tenant offboarding, and restore-from-backup, plus three the roadmap line did not name but the vault made necessary: rotating the secret-vault key, recovering from a lost key, and proving the audit trail has not been altered. **The ninth — incident response — is now DRAFTED (§13, 2026-08-21, Phase 41), explicitly marked as a draft pending Indian counsel review rather than left blank.** The owner decided a draft is less wasted effort than a blank page when that review eventually happens, on the explicit understanding a lawyer is expected to substantially rewrite it — particularly `[needs decision: who holds containment authority]` and `[needs decision: notification obligations under CERT-In 70B / the DPDP Rules]`, both left open rather than guessed at. Still not ticked `[x]` — that requires the counsel review this section itself says it is waiting on.)*
