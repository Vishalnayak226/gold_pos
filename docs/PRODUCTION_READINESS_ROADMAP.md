...
| Payment amount not bound to order | Order is not persisted; verify trusts the caller's `amount`. The gateway signature does not bind that submitted amount | Persist amount in paise, customer, currency, status, expiry; credit only stored amount after captured/paid confirmation |
| Manual UPI creates unverified credit | An authenticated customer can enter arbitrary reference text and immediately receive a ledger deposit | Record a pending claim; only verified reconciliation/manager approval can post credit; references unique |
